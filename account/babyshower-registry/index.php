...
$FormSubmitTo = "../bin/user.babyshower.reg.php?usrReg=new";
$userImagePath = '../babyshower-registry/' . str_replace('/', '.', $_SESSION['account']['refUserId']);
$editStage = "new";
        ...
        <link href='https://fonts.googleapis.com/css?family=Raleway' rel='stylesheet' type='text/css'>
                    ...
                    <body>
                        ...
                        for ($i = 0; $i < count($arrRegistries); $i++) {
                            if ($arrRegistries[$i] == "babyshower_registry") {
                                $BabyShowerRegistry = true;
                                break;
                            }
                        }
                        ...
                        if ($BabyShowerRegistry) {
                            //getDatabase variables
                            $FormSubmitTo = "../bin/user.babyshower.reg.php?usrReg=update";
                            $editStage = "repeat";
                            ...
                            try {
                                if ($DBMS->dbconnection_status) {
                                    ...
                                    if ($sqlSelect->rowCount() > 0) {
                                        ...
                                        $FormData[0] = splitEntry($row[2], 0, '*');
                                        $FormData[1] = splitEntry($row[2], 1, '*');
                                        if ($row[3] != NULL) {
                                            $FormData[2] = splitEntry($row[3], 0, '*');
                                            $FormData[3] = splitEntry($row[3], 1, '*');
                                        }
                                        ...
                                        $FormData[8] = $row[6]; //bslocation
                                        $FormData[11] = $row[9]; //bsdstatus
                                        ...
                                        <div class="large-12 columns row-fluid">
                                            ...
                                            <div class="large-4 columns" id="addPartner-cont" style="margin-left:5%">
                                                <div class="control-group" id="inputFNameGroup_add">
                                                    ...
                                                    <div class="controls">
                                                        <input type="text" id="inputFpartner" name="inputFpartner" placeholder="First Name" value="<?= $FormData[2] ?>">
                                                    </div>
                                                </div>

                                                <div class="control-group" id="inputMNameGroup_add">
                                                    <label class="control-label" for="inputMName_add">Partner&apos;s Middle Name (Optional)</label>
                                                    <div class="controls">
                                                        <input type="text" id="inputMName_add" name="inputMName_add" placeholder="Middle Name" value="<?= $FormData[3] ?>">
                                                    </div>
                                                </div>
                                            </div>     
                                        </div>
                        ...
                        $(document).ready(function () {
                            if ($('#editStage').val() == 'repeat') {
                                if ($("#selectData").val() != "") {
                                    ...
                                    $('#selectbabyshowerStatus').val(arrCheck[3]);
