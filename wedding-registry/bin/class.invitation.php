...
public function getPublicRegistries(){
	...
	try{
		if($invite_TEMPDB->dbconnection_status){
			...
			$sqlRegistry->execute(array('wedding_registry','public'));
...
private function strPublicRegEntries($userId){
	$invite_TEMPDB = $this->setDatabase('samedico_wedding_registry');
	$itemsCount = 0;
	try{
		if($invite_TEMPDB->dbconnection_status){
	...
	try{
		if($invite_TEMPDB->dbconnection_status){
			...
			if($sqlUser->rowCount()>0){
				...
				$sqlRegistry->execute(array($this->userId,'wedding_registry'));
				if($sqlRegistry->rowCount()>0){
					$row = $sqlRegistry->fetch(PDO::FETCH_ASSOC);
...
public function processImages(){
	 ...
	 $imageLink = '<img src="'.$_SESSION['page']['home_url'].'account/wedding-registry/'.$userId.'/'.$this->weddingImages.'" />';
	 return $imageLink;
}
...
public function getItems(){
	...
	if($sqlItems->rowCount()>0){
		...
		$_SESSION['arrItems'] = $this->arrItemQuantities;
	}else{
...
private function setHTMLContainer($details,$quantity,$itemCode){
	...
	$strHTML = $contHeading.'
		...
		';
		array_push($this->arrItemQuantities,$itemCode.'_'.($quantity - $quantityPurchased));
